...
async function Start() {
  ...
  // Max_id を取得する
  await RPA.Logger.info('現在のMax-id →' + TextData.slice(-1)[0][6]);
  const LastMax_id = Number(TextData.slice(-1)[0][6]) - 200;
  await RPA.Logger.info(`貼り付けするLastMax_id → ${LastMax_id}`);
  // スプレッドシートにデータを貼り付ける
  await DataPaste(TextData, String(LastMax_id));
}
...
async function GetTweetID(KeyWord, Max_id) {
  ...
  if (FirstData[0][4] != undefined) {
    ...
    const Max_id_low1 = Number(FirstData[0][4]);
